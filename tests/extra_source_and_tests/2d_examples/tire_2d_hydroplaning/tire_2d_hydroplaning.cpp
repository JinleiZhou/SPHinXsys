...
Real rim_thickness = resolution_ref;
Real gravity = 9.81;
Real U_f = 10.0;                                              
Real rho0_f = 1000.0;
Real c_f = 10.0 * U_f;
Real mu_f = 0.001;
Real water_length = 0.3;
Real water_height = 0.01;
Vec2d tire_observe_point(road_length*0.3,road_height+2*resolution_ref);
BoundingBoxd system_domain_bounds(Vec2d(-0.8, -0.5), Vec2d(3.0, 1.0));
...
public:
    explicit Tire(const std::string &shape_name) : MultiPolygonShape(shape_name) {
        multi_polygon_.addACircle(Vecd(road_length*0.3,road_height+R_outer), R_outer, 50, ShapeBooleanOps::add);
        multi_polygon_.addACircle(Vecd(road_length*0.3,road_height+R_outer), R_inner-rim_thickness, 50, ShapeBooleanOps::sub);
    }
};
...
public:
    explicit Road(const std::string &shape_name) : MultiPolygonShape(shape_name) {
        std::vector<Vecd> road_shape = {
            ...
            Vecd(road_length+0.1, road_height), Vecd(road_length+0.1, 0.0), Vecd(0.0, 0.0)
        };
...
public:
    explicit WaterBlock(const std::string &shape_name) : MultiPolygonShape(shape_name) {
        std::vector<Vecd> water_shape = {
            ...
            Vecd(water_length+0.5, road_height+water_height), Vecd(water_length+0.5, road_height), Vecd(0.5,road_height)
        };
...
MultiPolygon createRimBaseShape()
{
    ...
    multi_polygon.addACircle(Vecd(road_length*0.3,road_height+R_outer ),R_inner, 50, ShapeBooleanOps::add);
    multi_polygon.addACircle(Vecd(road_length*0.3,road_height+R_outer ),R_inner-rim_thickness, 50, ShapeBooleanOps::sub);
 ...
 class TireMotionConstraint : public MotionConstraint<BodyPartByParticle>
{
  public:
    ...
    Real v_underload = -0.0075/0.03 ; // Center descent speed = (3 × resolution drop under static vertical load) / descent time

    Real v_forward = 5.0;
    // Translation speed
    Vecd v_trans;
    if (t < 0.03) //descent time
    {
        v_trans = Vecd(0.0, v_underload);
    }
    else
    {
        v_trans = Vecd(v_forward, 0.0);
    }

    // Tire center
    Real cx = 0.3 * road_length;
    ...
    if (t < 0.03) 
    {
        angular_0 = 0.0;  // vertical compression only, no rotation.
    } 
    else 
    {
        angular_0 = v_forward/R_eff;
    } 

    // Rotation
    Real x = pos_[index_i][0] - cx;
    Real y = pos_[index_i][1] - cy;
    Vecd v_rot(angular_0 * y, -angular_0 * x);
...
int main(int ac, char *av[]) {
    ...
    // Observer on tire for VonMisesStress
    ObserverBody tire_stress_observer(sph_system, "TireStressObserver");
    tire_stress_observer.generateParticles<ObserverParticles>(StdVec<Vec2d>{tire_observe_point}); 
    ...
    ContactRelation tire_stress_observer_contact(tire_stress_observer, {&tire});
    ContactRelation insert_body_contact(tire, {&water_block});
    SurfaceContactRelation tire_road_contact(tire, {&road});
    ComplexRelation water_block_complex(water_block_inner, water_block_contact);
    SelfSurfaceContactRelation tire_self_contact(tire);
    
    //----------------------------------------------------------------------
    //	Define the main numerical methods used in the simulation.
    //----------------------------------------------------------------------
    BodyRegionByParticle wheel_rim(tire, makeShared<MultiPolygonShape>(createRimBaseShape()));
    Gravity gravity_(Vec2d(0.0,-gravity));
    SimpleDynamics<GravityForce<Gravity>> constant_gravity_water(water_block, gravity_);
    ...
    //----------------------------------------------------------------------
    Dynamics1Level<solid_dynamics::DecomposedIntegration1stHalf> insert_body_stress_relaxation_first_half(insert_body_inner);
    ...
    InteractionDynamics<solid_dynamics::SelfContactFactorSummation> tire_self_contact_density(tire_self_contact);
    InteractionWithUpdate<solid_dynamics::SelfContactForce> tire_self_contact_forces(tire_self_contact);
    ...
    write_recorded_tire_stress("VonMisesStress", tire_stress_observer_contact);
    ...
    write_recorded_tire_stress.writeToFile(number_of_iterations); 
    ...
    {
      write_recorded_tire_stress.generateDataBase(1.0e5);
    }
    else if (sph_system.RestartStep() == 0)
    {
      write_recorded_tire_stress.testResult();
    }
